...
export default function SuaDonPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    fetch(`/api/orders/${id}`).then((r) => r.json()).then((d) => setDonCu({
      ...
      items: d.items.map((it: { productId?: number; tenMon: string; coBanh?: string; cot?: string; kem?: string; mut?: string; topping?: string[]; soLuong: number; chuViet?: string; ghiChu?: string; giaBase?: number | null; gia: number; anhMau: string[] }) => ({
        ...it,
        cot: it.cot ?? '',
        kem: it.kem ?? '',
        mut: it.mut ?? '',
